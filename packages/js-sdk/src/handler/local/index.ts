import { Hook, RequestExtra } from "../proxy/hooks.js";
import {
    CallToolRequest,
    CallToolResult,
    InitializeRequest,
...
function wrapLocalResponse(upstream: Response): Response {
...
export function withLocalProxy(handler: (req: Request) => Promise<Response>, hooks: Hook[]) {
    return async (req: Request): Promise<Response> => {
        // Non-JSON passthrough
        if (!req.headers.get("content-type")?.includes("json")) {
            const upstream = await handler(req);
            return wrapLocalResponse(upstream);
        }

        // Parse JSON while preserving original body
        ...
        if (!originalRpc || Array.isArray(originalRpc)) {
            ...
            const upstream = await handler(new Request(req.url, { method: req.method, headers: forwardHeaders, body: originalBodyText ?? undefined }));
            return wrapLocalResponse(upstream);
        }
        ...
        // Generic handler reused by most methods
        const handleGeneric = async <TReq extends McpRequest, TRes extends McpResult>(
            ...
            try {
                ...
                if (hasModifications) {
                    upstream = await handler(new Request(req.url, {
                        method: req.method,
                    ...
                    upstream = await handler(new Request(req.url, {
                        method: req.method,
                        headers: forwardHeaders,
                        body: originalBodyText ?? undefined,
                    }));
                }
            } catch (e) {
                for (const h of hooks.slice().reverse()) {
                    const rr = await (runError(h, { code: 0, message: (e as Error)?.message || "local_handler_error" }, currentReq) || Promise.resolve(null));
                ...
                return jsonResponse({ jsonrpc: "2.0", id: (originalRpcLocal?.id as any) ?? 0, error: { code: -32000, message: "Local handler failed" } }, 502);
            }
            ...
            if (isStreaming) {
                ...
                try { text = await upstream.text(); } catch { return wrapLocalResponse(clonedUpstream); }
                try {
                ...
                } catch { return wrapLocalResponse(clonedUpstream); }
            } else if (isJson) {
                try { data = await upstream.json(); } catch { return wrapLocalResponse(clonedUpstream); }
            } else {
                return wrapLocalResponse(upstream);
            }
            ...
            if (maybeRpc && typeof maybeRpc === "object" && "jsonrpc" in maybeRpc) {
                if ("error" in maybeRpc) {
                    ...
                    return wrapLocalResponse(clonedUpstream);
                }
